...
where K: Hash + Eq,
    V: Hash +  Eq,
{
    pub fn new(start_dimension: usize, dimensions: usize) -> SubstitutionPartition<K, V> {
    ...
    V: Hash + Eq + Value + Window<W>,
{
    ...
    ///
    fn new(dimensions: usize, tolerance: usize) -> SubstitutionDB<W, V> {
        ...
        let mut partitions: Vec<SubstitutionPartition<W, V>> = Vec::with_capacity(head_count + tail_count);
        for i in 0..head_count {
            let start_dimension = i * head_width;
            let dimensions = head_width;
            let p: SubstitutionPartition<W, V> = SubstitutionPartition::new(start_dimension, dimensions);
    ...
    V: Hash + Eq + Value + Window<W>,
{
    ...
    fn ne(&self, other: &SubstitutionDB<W, V>) -> bool {
        return self.dimensions != other.dimensions ||
            self.tolerance != other.tolerance ||
            self.partition_count != other.partition_count; // ||
            //self.partitions.eq(&other.partitions);
...
mod test {
    ...
    #[test]
    fn find_missing_key() {
        let p: SubstitutionDB<usize, usize> = Database::new(8, 2);
    ...
    #[test]
    fn insert_first_key() {
        let mut p: SubstitutionDB<usize, usize> = Database::new(8, 2);
        let a = 0b11111111usize;
    ...
    fn find_permutations_of_multiple_similar_keys() {
        let mut p: SubstitutionDB<usize, usize> = Database::new(8, 4);
    ...
    fn find_permutation_of_inserted_key() {
        ...
        for start_dimensions in start_dimensions_seq.take(1000usize) {
            let mut p: SubstitutionDB<usize, usize> = Database::new(dimensions, max_hd);
            let a = 0b11111111usize;
    ...
    fn stability_under_load() {
        ...
        let mut p: SubstitutionDB<usize, usize> = Database::new(16, 4);
    ...
    fn idempotent_read() {
        fn prop(a: usize, b: usize, c: usize) -> quickcheck::TestResult {
            ...
            let mut p: SubstitutionDB<usize, usize> = Database::new(std::usize::BITS as usize, 4);
            p.insert(a.clone());
            p.insert(b.clone());
            p.insert(c.clone());
            p.remove(&c);
