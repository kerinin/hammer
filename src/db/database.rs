...
impl<V: Value, S: Store<V, V>> Database<S> {
     ...
     */
    pub fn new(bits: u64, tolerance: u64) -> Database<S> {
        ...
        let head_width = Ratio::new(bits, partition_count).ceil().to_integer() as u64;
        let tail_width = Ratio::new(bits, partition_count).floor().to_integer() as u64;
...
mod test {
    ...
    #[test]
    fn partition_evenly() {
        let a: Database<HashMapSet<u64, u64>> = Database::new(32, 5);
        let b = Database {
            bits: 32,
            tolerance: 5,
            partition_count: 4,
    ...
    fn partition_unevenly() {
        let a: Database<HashMapSet<u64, u64>> = Database::new(32, 7);
        let b = Database {
            bits: 32,
            tolerance: 7,
            partition_count: 5,
    ...
    fn partition_too_many() {
        let a: Database<HashMapSet<u64, u64>> = Database::new(4, 8);
        let b = Database {
            bits: 4,
            tolerance: 8,
            partition_count: 3,
    ...
    #[test]
    fn partition_zero() {
        let a: Database<HashMapSet<u64, u64>> = Database::new(32, 0);
        let b = Database {
            bits: 32,
            tolerance: 0,
            partition_count: 1,
    ...
    fn partition_with_no_bytes() {
        let a: Database<HashMapSet<u64, u64>> = Database::new(0, 0);
        let b = Database {
            bits: 0,
            tolerance: 0,
            partition_count: 1,
    ...
    #[test]
    fn find_missing_key() {
        let mut p: Database<HashMapSet<u64, u64>> = Database::new(8, 2);
        let a = 0b11111111u;
        let keys = p.get(a);
    ...
    fn find_permutations_of_multiple_similar_keys() {
        let mut p: Database<HashMapSet<u64, u64>> = Database::new(8, 4);
        let a = 0b00000000u;
        let b = 0b10000000u;
        let c = 0b10000001u;
        let d = 0b11000001u;
    ...
    fn find_permutation_of_inserted_key() {
        ...
        let bits = 8u;
        let max_hd = 3u;
        let shifts_seq = rng1.gen_iter::<u64>()
            .map(|i| sample(&mut rng2, range(0, bits), i % max_hd));

        for shifts in shifts_seq.take(1000u) {
            let mut p: Database<HashMapSet<u64, u64>> = Database::new(bits, max_hd);
            let a = 0b11111111u;
    ...
    fn stability_under_load() {
        ...
        let mut p: Database<HashMapSet<u64, u64>> = Database::new(16, 4);
        ...
        for i in seq.take(100000u) {
            if expected_present[i as u64] {
                p.remove(i as u64);
                expected_present[i as u64] = false;
                expected_absent[i as u64] = true;
            } else {
                p.insert(i as u64);
                expected_present[i as u64] = true;
                expected_absent[i as u64] = false;
            }
